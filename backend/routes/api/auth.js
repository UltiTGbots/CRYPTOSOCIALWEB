...
router.post("/signup", signupValidator, async (req, res) => {
  ...
  try {
    let userCount = await User.find().count() || 9999;
    let user = await User.findOne({ email });
    let checkUsername = await User.findOne({ username });
      let referid = await User.findOne({ profile_no: refred_id });
 ...
 // console.log(user, checkUsername);
    ...
    let latestRecord = await User.findOne().sort({ createdAt: -1 });
    if (latestRecord) {
      if (latestRecord.profile_no < 10000) {
        userCount = 9999
      } else {
        userCount = latestRecord.profile_no
      }
    } 
    ...
    if (profileType === "personal") {
      ...
      if (refred_id) {
         if (!referid) return res.send({ status: 404, message: "Entered referral profile ID is invalid." })
         newUser.following.unshift(referid._id);
        referid.followers.unshift(newUser._id);
        await referid.save();
      }
    } else {
...
const cronjon = new schedule.scheduleJob(cronExpression, async function () {
  try {
    ...
    userdata.forEach(async (userdata) => {
      const userdate = userdata.createdAt.getDate();
      const currentDate = new Date();
      const oneMonthLater = new Date(userdata.createdAt);
      if (userdata.profileType === 'personal') {
        // if (userdata.subscriptionType.subType === "FREE") {
        if (userdate && currentDate.getDate() === userdata.createdAt.getDate() && currentDate >= oneMonthLater) {
          if (userdata.subscriptionType.subType === "BUSINESS") {
            const result = await User.updateOne(
              { _id: userdata._id },
              { $set: { 'subscriptionType.reelCoin': userdata.subscriptionType.reelCoin + 50 } }
            );

            await sendFirebaseNotifications(
              `Congratulations, ${userdata.firstName} ${userdata.lastName}! You earned 50 coins!`,
              userdata.fcmToken,
              JSON.stringify(userdata),
              "chat"
            );
            console.log(`you earned +50 points`)
          } else if (userdata.subscriptionType.subType === "GOLD") {
            const result = await User.updateOne(
              { _id: userdata._id },
              { $set: { 'subscriptionType.reelCoin': userdata.subscriptionType.reelCoin + 20 } }
            );

            await sendFirebaseNotifications(
              `Congratulations, ${userdata.firstName} ${userdata.lastName}! You earned 20 coins!`,
              userdata.fcmToken,
              JSON.stringify(userdata),
              "chat"
            );
            ...
            const result = await User.updateOne(
              { _id: userdata._id },
              { $set: { 'subscriptionType.reelCoin': userdata.subscriptionType.reelCoin + 10, 'subscriptionType.subType': userdata.subscriptionType.subType = "FREE" } }
            );

            await sendFirebaseNotifications(
              `Congratulations, ${userdata.firstName} ${userdata.lastName}! You earned 10 coins!`,
              userdata.fcmToken,
              JSON.stringify(userdata),
              "chat"
            );
          ...
          console.log("One Month is not passed yet!")
        }
        // } else {
        //   console.log("You don't have free account!")
        // }
        ...
        if (userdate && currentDate.getDate() === userdata.createdAt.getDate() && currentDate >= oneMonthLater) {
          if (userdata.subscriptionType.subType === "BUSINESS") {
            ...
            await sendFirebaseNotifications(
              `Congratulations, ${userdata.firstName}! You earned 50 coins!`,
              userdata.fcmToken,
              JSON.stringify(userdata),
              "chat"
            );
            ...
            await sendFirebaseNotifications(
              `Congratulations, ${userdata.firstName}! You earned 20 coins!`,
              userdata.fcmToken,
              JSON.stringify(userdata),
              "chat"
            );
            ...
            const result = await User.updateOne(
              { _id: userdata._id },
              { $set: { 'subscriptionType.reelCoin': userdata.subscriptionType.reelCoin + 20, 'subscriptionType.subType': userdata.subscriptionType.subType = "FREE" } }
            );
            ...
            console.log("You earned +20 points business account")
          }
        } else {
          console.log("One Month is not passed yet!")
        }
        // } else {
        //   console.log("Bussiness user You don't have free account!")
        // }
      }
    })
  } catch (error) {
...
router.post("/social", async (req, res) => {
  const { email, firstName, lastName, refred_id } = req.body;
  try {
    let userCount = await User.find().count() || 9999;
    let user = await User.findOne({ email });
//    let findprofileid = await User.findOne({ profile_no: refred_id });
 ...
 if (user) {
     ...
     if (refred_id) {
     let findprofileid = await User.findOne({ profile_no: refred_id });
        if (!findprofileid) return res.send({ status: 404, message: "Entered referral profile ID is invalid." })  
      await user.updateOne({ refred_id: refred_id });
         user.following.unshift(findprofileid._id)
        findprofileid.followers.unshift(user._id);
  await user.save()
        await findprofileid.save()       
      }
      jwt.sign(
        payLoad,
        "mysecrettoken",
        ...
        (err, token) => {
          ...
          res.json({ token, status: 200, user, userstatus: 0 });
        }
      );
    } else {
      ...
      user = new User({
        ...
        firstName,
        lastName,
//        refred_id: findprofileid?.profile_no,

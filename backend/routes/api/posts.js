...
router.post("/", auth, async (req, res) => {
  const { text, postType, location, media, mimeType , isReelMail, postCategory } = req.body;
  try {
...
router.get("/watchVideo/:postId", auth, async (req, res) => {
  ...
  try {
    ...
    await sendFirebaseNotifications(
      `${loggedInUserInfo?.username || loggedInUserInfo?.firstName} watched your reel From the feed`,
       user.fcmToken,
       JSON.stringify(postReelEntry),
       'post'
        )
    var userNotification = new Notification({
      message: `${loggedInUserInfo?.username || loggedInUserInfo?.firstName} watched your reel from the feed`,
      post: postId,
      user: user._id,
      type: "post",
...
router.get("/user/:id", auth, async (req, res) => {
  try {
    ...
    const post = await Post.find({ user: user._id }).populate("user").populate("postCategory").populate({
      path: "comments.user",
      model: "user",
    });
    return res.json({ post, status: 200 });
...
router.get("/:id", auth, async (req, res) => {
  try {
    const post = await Post.findById(req.params.id).populate("user").populate("postCategory");
    if (!post) {

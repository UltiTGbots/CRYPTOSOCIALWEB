...
router.get("/referList", auth, async (req, res) => {
  try {
    const page = req.query.page || 1; // Default to page 1 if not specified
    const perPage = req.query.perPage || 10;
    const skip = (page - 1) * perPage;
    const user = await User.findById(req.user.id);
    const findref = await User.find({
      $and: [
        { refred_id: { $ne: 0 } },
        { refred_id: user.profile_no }
      ]
    }).select("username profile_no media firstName lastName").sort({ _id: -1 }).skip(skip).limit(perPage).exec()
    ...
    return res.send({ detils: findref, userCount: Count })
  } catch (error) {
    console.log(error)
  }
})
